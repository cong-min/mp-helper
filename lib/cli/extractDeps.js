...
const cssImport = require('./utils/cssImport');
...
module.exports = (options = {}) => {
    ...
    async function lookupDeps(file) {
        ...
        let strings = [];
        if (['.wxss', '.css', '.less'].includes(file.extname)) {
            file.depType = 'css';
            // css 找出代码内的 @import
            strings = cssImport.find(fileContent);
        } else if (['.js', '.ts', '.wxs'].includes(file.extname)) {
            file.depType = 'js';
            // js 找出代码内的 import require 等
            const result = konan(fileContent);
        ...
        let npms = strings.map(expression => ({
            ...
            path: resolveFrom.silent(dirname, expression), // 源文件路径
        }));
        ...
        npms = npms.reduce((obj, npm) => {
            ...
            if (packageName === expression) {
                ...
                targetId = path.join(expression, `index${path.extname(npm.path) || '.js'}`);
        ...
        // 对于 css 中引用了 npm 中的css, 小程序无法自动找到 miniprogram_npm 路径, 需对内容路径进行替换
        if (file.depType === 'css') {
            fileContent = cssImport.replace(fileContent, npms, {
                fileInfo: file, // 原文件信息
                depPathPrefix: 'miniprogram_npm', // 依赖前缀
            });
    ...
    async function extractDeps(file, enc, next) {
        ...
        try {
            ...
            logger.error('解析', relative(file.path));
            err.filename = relative(file.path);
